...
from src.utils.store_utils import get_store
...
@router.post("/prediction/with_caching")
async def predict_from_id_cached(req: SegmentationFromIdRequest,
    ...
    # Extract corresponding part of the image
    img_path = store.get(req.image_id)
    if img_path:
        img = plt.imread(img_path)

    else:
        img_path = download_and_cache_image(req.image_id, store, settings)
        img = plt.imread(img_path)

    height = img[0]

    try:
        formatted_image, x, y, cropped_width, cropped_height = extract_img_region(img, box_global)
